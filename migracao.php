...
function arr_replace($search,$replace,$array){
	...
	if( is_array($array) ){
		...
		foreach ($array as $key => $value) {
			...
			if( is_array($value) ){
				$newArray[$key] = arr_replace($search, $replace, $value);
			} else {
...
if(isset($_POST['url_antiga']) && isset($_POST['url_nova'])){
	...
	if (strlen($_POST['url_antiga']) < 5 || strlen($_POST['url_nova']) < 5){
...
$searchString = array( str_replace('http://', '', $_POST['url_antiga']), urlencode(str_replace('http://', '', $_POST['url_antiga'])) );
$novaURL = array( str_replace('http://', '', $_POST['url_nova']), urlencode(str_replace('http://', '', $_POST['url_nova'])) );
...
class st2Class {
    ...
    public function __construct($obj) {
    	...
    	foreach ($keys as $key => $value) {
    		...
    		if($key != 'url'){
    			$this->$key = str_replace($searchString[0], $novaURL[0], $obj->$key);
    		}else{
    			$this->$key =  $obj->$key;
    		}
    	}
...
class st3Class {
    ...
    public function __construct($obj) {
    	...
    	foreach ($keys as $key => $value) {
    		...
    		if($key != 'url'){
    			$this->$key = str_replace($searchString[1], $novaURL[1], $obj->$key);
    		}else{
    			$this->$key =  $obj->$key;
    		}
    	}
...
class st4Class {
    ...
    public function __construct($obj) {
    	...
    	foreach ($keys as $key => $value) {
    		...
    		if($key != 'url'){
    			$this->$key = str_replace($optionAntigo, $optionNovo, $obj->$key);
    		}else{
    			$this->$key =  $obj->$key;
    		}
    	}
...
if (strlen($_POST['url_antiga']) > 5 && strlen($_POST['url_nova']) > 5){
	...
	foreach ($searchString as $sKey => $searchValue) {
		$query = $wpdb->get_results("SELECT * FROM {$wpdb->posts} WHERE guid LIKE '%{$searchString[$sKey]}%'");
		echo count($query)." post meta encontrado(s)<hr />";
		echo $wpdb->posts;
		echo "<br><br><br>";

		//Atualizar referencias de imagens e links nos posts
		$update = mysqli_query($con, "UPDATE {$wpdb->posts} SET post_content = replace(post_content, '{$searchString[$sKey]}','{$novaURL[$sKey]}')");
		$update2 = mysqli_query($con, "UPDATE {$wpdb->posts} SET guid = replace(guid,'{$searchString[$sKey]}','{$novaURL[$sKey]}')") or die(mysqli_error($con));
	...
	foreach ($searchString as $sKey => $searchValue) {
		...
		//Post Meta
		$query = $wpdb->get_results("SELECT * FROM {$wpdb->postmeta} WHERE meta_value LIKE '%{$searchString[$sKey]}%'");
		echo count($query)." post meta encontrado(s)<hr />";
		...
		foreach ($query as $key => $q1) {
			...
			if(is_array($q2)){
				...
				$q2 = arr_replace( $searchString[$sKey], $novaURL[$sKey], $q2 );
			} 
				...
				$q2 = str_replace($searchString[$sKey], $novaURL[$sKey], $q2);
	...
	foreach ($searchString as $sKey => $searchValue) {
		...
		// Comment Meta
		$query2 = $wpdb->get_results("SELECT * FROM {$wpdb->commentmeta} WHERE meta_value LIKE '%{$searchString[$sKey]}%'");
	...
	$revslider = mysqli_query($con, 'select 1 from '.$wpdb->prefix.'revslider_sliders');

	if( $revslider ){
		// RevSlider
		$settingsParams = $wpdb->get_var("SELECT params FROM {$wpdb->prefix}revslider_settings LIMIT 1");
		$newSettingsParams = serialize( arr_replace($searchString[0], $novaURL[0], $settingsParams) );

		$wpdb->query("UPDATE {$wpdb->prefix}revslider_settings set params = '{$newSettingsParams}'");

		$slidersParams = $wpdb->get_var("SELECT params FROM {$wpdb->prefix}revslider_sliders LIMIT 1");
		$newSlidersParams = serialize( arr_replace($searchString[0], $novaURL[0], $slidersParams) );

		$wpdb->query("UPDATE {$wpdb->prefix}revslider_sliders set params = '{$newSlidersParams}'");


		$slidesParams = $wpdb->get_var("SELECT params FROM {$wpdb->prefix}revslider_slides LIMIT 1");
		$newSlidesParams = serialize( arr_replace($searchString[0], $novaURL[0], $slidesParams) );

		$wpdb->query("UPDATE {$wpdb->prefix}revslider_slides set params = '{$newSlidesParams}'");
		

		$slidesLayers = $wpdb->get_var("SELECT layers FROM {$wpdb->prefix}revslider_slides LIMIT 1");
		$newSlidesLayers = serialize( arr_replace($searchString[0], $novaURL[0], $slidersParams) );

		$wpdb->query("UPDATE {$wpdb->prefix}revslider_slides set layers = '{$newSlidesLayers}'");
	...
	if (strlen($optionAntigo) > 2 && strlen($optionNovo) > 2){
		...
		foreach ($query2 as $key => $q1) {
			...
			if(is_array($q2)){
				...
				$q2 = arr_replace($optionAntigo, $optionNovo, $q2);
...
$wpdb->query("UPDATE {$wpdb->postmeta} set meta_value = replace(meta_value, 'st3Class', 'stdClass')");

$wpdb->query("UPDATE {$wpdb->commentmeta} set meta_value = replace(meta_value, 'st3Class', 'stdClass')");
...
<footer>
	Criado por <a href="http://trii.com.br">Trii</a> | Vers&atilde;o 2.4.2 | 14/03/2016
	<!--
		RELEASE NOTES:
			• 2.4.1 - 12/11/2015 - Versão Inicial
			• 2.4.2 - 14/03/2016 - Correção revslider
	-->
</footer>